...
public class OperationsService {
	...
	public static void prepareFirstMachineOperations(List<Task> tasks, final List<Maintenance> machineOneMaintenances) {
		...
		for (Task task : tasks) {
			Operation operation = task.getFirst();//getTaskOperationForMachine(task, Machine.ONE);
	...
	public static void prepareSecondMachineOperations(List<Task> tasks) {
		int lastOperationEndTime = 0;
		for (Task task : tasks) {
			Operation operation = task.getSecond();
			int firstMachineLastOperationEndTime = task.getFirst().getEnd();
